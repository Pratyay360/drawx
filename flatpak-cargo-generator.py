...
async def generate_sources(
    ...
    if tomlkit is not None:
        config_contents = tomlkit.dumps({"source": cargo_vendored_sources})
    else:
        config_lines = []
        for src_name, src_val in cargo_vendored_sources.items():
            config_lines.append(f"[source.{src_name}]")
...
def main() -> None:
    ...
    if args.yaml and YAML_AVAIL:
        ...
        with open(outfile, "w", encoding="utf-8") as out:
            json.dump(generated_sources, out, indent="\t", sort_keys=False)
            out.write("\n")
