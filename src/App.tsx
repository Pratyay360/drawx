...
import { Check, FileText, Grid2x2, List, Pencil, Search, Trash2, X } from "lucide-react";
            ...
            <Grid columns={{ minWidth: 280, max: 3 }} gap={3}>
              {filteredCanvases.map((canvas) => (
                <ClickableCard
                  ...
                  padding={3}
                  onClick={() => {
                    if (editingId !== canvas.id) navigate(`/canvas/${canvas.id}`);
                  }}
                >
                  <VStack gap={2}>
                    <HStack justify="between" align="center">
                    ...
                    {editingId === canvas.id ? (
                      ...
                      <Text type="supporting">{formatDate(canvas.updatedAt)}</Text>
                    )}
                  </VStack>
                </ClickableCard>
              ))}
