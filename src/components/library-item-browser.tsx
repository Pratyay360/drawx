...
function LibraryItemThumbnail({ itemId, elements }: LibraryItemThumbnailProps) {
  ...
  if (failed) {
    return <Icon icon="lucide:image-off" className="w-5 h-5 text-muted-foreground" />;
  }
  return <Icon icon="lucide:loader-2" className="w-5 h-5 animate-spin text-muted-foreground" />;
}
...
function getItemSearchText(item: any, index: number, itemNames: string[]): string {
  const parts: string[] = [];
      ...
      <CardContent className="space-y-4">
        {hasContent ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {items.map(({ item, name }, index) => (
                  ...
                  >
                    ...
                    <p className="mt-1.5 text-xs text-muted-foreground truncate">{name}</p>
                  </div>
                ))}
              </div>
            ) : (
          ...
          <div className="flex flex-col items-center gap-3 py-10 text-center">
            <Icon icon="lucide:download-cloud" className="w-8 h-8 text-muted-foreground" />
            <div>
