...
/**
 * Theme picker: switches the app theme (Butter / Neutral / Matcha / Stone / Y2K / Gothic)
 * and the color mode (light / dark / system). Copied from drawy/src/components/theme-toggle.tsx.
 */
export function ThemeToggle() {
  const { themeName, modePreference, setThemeName, setMode } = useTheme();
  const current = themeRegistry[themeName];

  const checkFor = (selected: boolean) => (selected ? <Icon icon={Check} size="sm" /> : undefined);
