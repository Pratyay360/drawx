 ...
 * The Astryx `<Theme>` provider consumes both via ThemedApp.
 */
export function useTheme() {
  ...
  // Back-compat aliases — old code used `theme`/`setTheme` for light|dark only.
  const setTheme = useCallback((next: ColorMode) => {
    setStateInternal({ modePreference: next });
  }, []);
  ...
  return {
    ...
    setMode,
    toggleMode,
    // Legacy API (mode-only) — kept for existing call sites during migration
    theme: getEffectiveMode(snapshot.name, snapshot.modePreference) as ColorMode,
    setTheme,
    toggleTheme,
  };
}

// Re-render + re-apply document state when the OS preference flips while the user's preference is "system".
