...
export function sanitizeExcalidrawAppState(
	...
	if (
		appState === null ||
		Object.prototype.toString.call(appState) !== "[object Object]" ||
		Array.isArray(appState)
	) {
		return {};
	}

	// Strip `collaborators` (not serializable) and `theme` (must always come
	// from the app's active theme via the `theme` prop, never from stale storage).
	const {
...
function normalizeCanvas(canvas: RawCanvas): Canvas {
	return {
		...
		createdAt:
			canvas.createdAt ?? canvas.created_at ?? new Date().toISOString(),
		updatedAt:
			canvas.updatedAt ?? canvas.updated_at ?? new Date().toISOString(),
...
export function isTauri(): boolean {
	...
	// SAFETY: Tauri injects __TAURI_INTERNALS__ onto window at runtime; the
	// cast narrows window to the augmented shape to check presence without
	// widening the global type to unknown.
	const tauriWindow = globalThis.window as Window & {
...
function getLocalStorageCanvases(): Canvas[] {
	try {
		...
		const canvases: unknown = data ? JSON.parse(data) : [];
