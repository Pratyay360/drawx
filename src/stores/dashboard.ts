...
import { createCanvas, deleteCanvas, listCanvases, updateCanvasTitle } from "../services/tauri.ts";
...
export const useDashboardStore = create<DashboardStore>()((set, get) => ({
  ...
  renameCanvas: async (id, title) => {
    ...
    try {
      await withTimeout(updateCanvasTitle(id, title.trim()), 12000, "Rename canvas");
      set({ editingId: null });
      globalThis.dispatchEvent(new Event("canvas-updated"));
      await get().reload();
    } catch (error) {
