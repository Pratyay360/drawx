...
function parseUpdateEvent(event: Event): UpdateInfo | null {
  ...
  if (detail === null || Object.prototype.toString.call(detail) !== "[object Object]") return null;
  const version = detail.version;

  const notes = detail.notes;
  if (Object.prototype.toString.call(version) !== "[object String]") return null;
  if (notes !== null && Object.prototype.toString.call(notes) !== "[object String]") return null;
...
export async function installUpdate(version: string): Promise<void> {
  try {
    ...
    if (!update || update.version !== version) return;
